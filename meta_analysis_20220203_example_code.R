...
setwd("C:/Users/lutang/...") # set to the work directory
...
dd <- read.csv("input/Aim2_UDS_logistic_model_Either_UDS_v2.csv")

### can print it in R to see
print(dd)
View(dd) # works for RStudio
...
### prepare the data
dd$Parameter <- paste0(dd$Variable, ifelse(dd$ClassVal0 == "", "", "_"), dd$ClassVal0) # create a new name for the variables for convenience
...
pdf(file = paste0("MA_Forest_Plots_20220203.pdf"), # START saving plots in a pdf file
...
outdata$description <- paste0("(global OR ", round(outdata$OR_estimate, 2), ", ",
                              ...
                              ")")
write.csv(outdata, paste0("MA_META_RESULT_20220203.csv"), row.names = FALSE, na = "")
...
ggplotdata$variables <- factor(ggplotdata$variables,      # Reordering group factor levels
                               levels = c("Age Index", "Male", "Hispanic", "Non Hisp. Black", "Other Race/Ethnicity", "Children",             
                                          "Disabled", "Rural", "ID", "MISUD", "MEDCOMP", "Any OD Pre",         
                                          "OD Prof Pr Dx Oud Pr", "Either UDS Pre", "2019", "2018", "2017", "2014 [REF]", 
                                          "Non Disabled Adult [REF]", "Non Hisp. White [REF]"
                               ))
levels(ggplotdata$variables)

# create variable groups for clustered display
ggplotdata$variablesgroup <- as.factor(c("Age", "Sex", "Race/Ethnicity", "Race/Ethnicity", "Race/Ethnicity", 
...
levels(ggplotdata$variablesgroup)
ggplotdata$variablesgroup <- factor(ggplotdata$variablesgroup,      # Reordering group factor levels
                                    levels = c("Age", "Sex", "Race/Ethnicity", "Eligibility Group", "Region", "Year", 
                                               "ID", "MISUD", "MEDCOMP", "Any OD Pre", "OD Prof Pr Dx Oud Pr", "Either UDS Pre"))
levels(ggplotdata$variablesgroup)

# view the data used in ggplot
View(ggplotdata)
...
# save the plot
ggsave(file = paste0("MA_Global_Log_OR_Plot_20220203", ".pdf"), 
       width = 8.5, height = 11, scale = 0.8, device="pdf")
...
#### ADDING NEW INFORMATION TO THE FIGURE
# For example, we add the following
# ggplotdata$addtext <- paste0("OR: ", round(ggplotdata$OR_estimate, 2), ", ",
#                              "95% CI: ", round(ggplotdata$OR_ci_lb, 2), "-", round(ggplotdata$OR_ci_ub, 2))
...
# add the text to it
p + geom_text(aes(label = addtext, x = 0.0001), hjust = 0)
p + geom_text(aes(label = addtext, x = 0.06), hjust = 1)
p + geom_text(aes(label = addtext, x = 0.002), hjust = 0.5)

ggsave(file = paste0("MA_Global_OR_Plot_Log2_scale_20220203_with_text", ".pdf"), 
       width = 8.5, height = 11, scale = 0.8, device="pdf")
